...
class Schema extends \Laravel\Database\Schema {
	...
	{
		call_user_func($callback, $table = new \Database\Schema\Table($table));
	...
	{
		$table = new \Database\Schema\Table($table);
	...
	public static function grammar(\Laravel\Database\Connection $connection)
	{
		...
		{
			case 'mysql':
				return new \Database\Schema\Grammars\MySQL($connection);

			case 'pgsql':
				return new \Database\Schema\Grammars\Postgres($connection);

			case 'sqlsrv':
				return new \Database\Schema\Grammars\SQLServer($connection);

			case 'sqlite':
				return new \Database\Schema\Grammars\SQLite($connection);
		}
